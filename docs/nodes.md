...
Concluída a instalação e ativação do `kubelet`, os nós informam seu status para o API Server e passam a constar no famoso comando `kubectl get nodes` como estado `NotReady`. Eles só transitarão para `Ready` na próxima etapa: a instalação dos Addons de Rede (CNI). O `kube-proxy` é inicializado logo após o provisionamento inicial, junto com a camada de operações.
